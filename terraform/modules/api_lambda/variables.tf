...
variable "streaming_reserved_concurrency" {
  description = "Reserved concurrency cap for the streaming Lambda (cost/abuse circuit-breaker). -1 = unreserved."
  type        = number
  default     = -1
}

variable "enable_streaming_waf" {
  description = "Whether to create a WAF Web ACL (rate limiting) in front of the streaming CloudFront distribution"
  type        = bool
  default     = false
}
